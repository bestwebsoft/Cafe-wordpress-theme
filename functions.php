...
class Cafe_Recent_Comments extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of comments to show:', 'cafe' ); ?></label>
...
class Cafe_Recent_Posts extends WP_Widget {
			...
			<ul>
				<?php while ( $rec_post_request->have_posts() ) :
					...
					<li>
						...
						<p> <?php _e( 'posted by', 'cafe' ); ?>
							<span class="cafe_lowercase"> <a href="<?php the_author_link(); ?>"> <?php the_author() ?> </a></span> <?php _e( 'in', 'cafe' ); ?>
							<span class="cafe_lowercase"><?php the_category( ', ' ) ?><span></p>
						<?php if ( $show_date ) : ?>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts to show:', 'cafe' ); ?></label>
			<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" />
		</p>
		<p>
			<input class="checkbox" type="checkbox" <?php checked( $show_date ); ?> id="<?php echo $this->get_field_id( 'show_date' ); ?>" name="<?php echo $this->get_field_name( 'show_date' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Display post date?', 'cafe' ); ?></label>
		</p>
	<?php }
} /* Widget added */
	...
	if ( ! is_front_page() ) :
		...
		if ( is_archive() ) :
			if ( is_category() ) :
				...
				echo $devider . '<div> ' . __( 'Category:', 'cafe' ) . '&nbsp;';
				print_r( $this_cat->name );
				echo ' </div> ';
			elseif ( is_author() ) :
				echo $devider;
				the_author_posts_link();
			elseif ( is_tag() ) :
				echo $devider . '<div> ' . __( 'Tag: ', 'cafe' ) . '&nbsp;';
				single_tag_title();
				echo ' </div> ';
			else :
				echo $devider . '<div> ' . __( 'Archive for:', 'cafe' ) . '&nbsp;';
				wp_title( ' ', true, 'right' );
				echo ' </div> ';
			endif;
		endif;
